...
final class ASTParser implements ASTParserConstants {
    ...
    private void pushJavadoc() {
        // Will not use CommonTokenAction to handle JavaDoc
    ...
    private List<Comment> getComments() {
        // Will not use CommonTokenAction to save comments     
    ...
    private final Comment getComment() {
        ...
        Comment prev = null;
        while (tmp_t != null) {
            if(tmp_t.kind == JAVA_DOC_COMMENT) {
                cmmt = new JavadocComment(tmp_t.beginLine, tmp_t.beginColumn, tmp_t.endLine, tmp_t.endColumn, tmp_t.image.substring(3, tmp_t.image.length()-2));
            } else if(tmp_t.kind == SINGLE_LINE_COMMENT) {
                cmmt = new LineComment(tmp_t.beginLine, tmp_t.beginColumn, tmp_t.endLine, tmp_t.endColumn, tmp_t.image.substring(2));
            } else if(tmp_t.kind == MULTI_LINE_COMMENT) {
                cmmt = new BlockComment(tmp_t.beginLine, tmp_t.beginColumn, tmp_t.endLine, tmp_t.endColumn, tmp_t.image.substring(2, tmp_t.image.length()-2));
            }
            cmmt.setComment(prev);
            prev = cmmt;
            tmp_t = tmp_t.next;
        }
        // The above loop now walks the special token chain in the forward
        // direction linking them in the process. 
...
CompilationUnit tmp = new CompilationUnit(line == -1 ? 0 : line, column, token.endLine, token.endColumn,pakage, imports, types, getComments());
      tmp.setComment(cmmt);
      {if ("" != null) return tmp;}
...
EnumConstantDeclaration tmp = new EnumConstantDeclaration(line, column, token.endLine, token.endColumn,popJavadoc(), annotations, name, args, classBody);
      tmp.setComment(cmmt);
      {if ("" != null) return tmp;}
    throw new Error("Missing return statement in function");
  }
...
int line = modifier.beginLine;
      ...
      FieldDeclaration tmp = new FieldDeclaration(line, column, token.endLine, token.endColumn, popJavadoc(), modifier.modifiers, modifier.annotations, type, variables);
      tmp.setComment(cmmt);
      {if ("" != null) return tmp;}
    throw new Error("Missing return statement in function");
  }
...
if (exConsInv != null) {
        ...
        ConstructorDeclaration tmp = new ConstructorDeclaration(line, column, token.endLine, token.endColumn,popJavadoc(), modifier.modifiers, modifier.annotations, typeParameters, null, parameters, throws_, new BlockStmt(bbLine, bbColumn, token.endLine, token.endColumn, stmts));
    tmp.setNameExpr(name);
        tmp.setComment(cmmt);
        {if ("" != null) return tmp;}
    throw new Error("Missing return statement in function");
...
InitializerDeclaration tmp = new InitializerDeclaration(line, column, token.endLine, token.endColumn,popJavadoc(), isStatic, block);
      tmp.setComment(cmmt);
      {if ("" != null) return tmp;}
    throw new Error("Missing return statement in function");
  }
...
ret.setComment(cmmt);
  ...
  final public Expression NullLiteral() throws ParseException {Comment cmmt = getComment();
...
int line = modifier.beginLine;
  ...
  final public EmptyStmt EmptyStatement() throws ParseException {Comment cmmt = getComment();
    jj_consume_token(SEMICOLON);
...
int line = modifier.beginLine;
    int column = modifier.beginColumn;
    { if (line == -1) {line=type.getBeginLine(); column=type.getBeginColumn();} }
    AnnotationMemberDeclaration tmp = new AnnotationMemberDeclaration(line, column, token.endLine, token.endColumn, popJavadoc(), modifier.modifiers, modifier.annotations, type, name, defaultVal);
    tmp.setComment(cmmt);
    {if ("" != null) return tmp;}
    throw new Error("Missing return statement in function");
  }
